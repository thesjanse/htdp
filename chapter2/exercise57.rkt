...
; ground and the rocket reference point (its height)
...
; ground and the rocket
(define (get-y-pos lrcd)
    (+ ROCKET-CENTER (fly lrcd)))

(check-equal? (get-y-pos "resting") ROCKET-CENTER)
(check-equal? (get-y-pos -3) (+ ROCKET-CENTER (fly -3)))
(check-equal? (get-y-pos -1) (+ ROCKET-CENTER (fly -1)))
(check-equal? (get-y-pos 0) (+ ROCKET-CENTER (fly 0)))
(check-equal? (get-y-pos 1) (+ ROCKET-CENTER (fly 1)))
(check-equal? (get-y-pos 2) (+ ROCKET-CENTER (fly 2)))
...
(define (render lrcd)
    (place-image ROCKET ROCKET-X (- HEIGHT (get-y-pos lrcd)) BACKGROUND))

(check-equal? (render -3)
    (place-image ROCKET ROCKET-X (- HEIGHT (get-y-pos -3)) BACKGROUND))
(check-equal? (render 0)
    (place-image ROCKET ROCKET-X (- HEIGHT (get-y-pos 0)) BACKGROUND))
(check-equal? (render 9)
    (place-image ROCKET ROCKET-X (- HEIGHT (get-y-pos 9)) BACKGROUND))
...
; LRCD -> Boolean
; stop simulation when rocket leaves the canvas
(define (stop-sim? lrcd)
    (cond
        [(and (string? lrcd)
              (string=? lrcd "resting")) #f]
        [(<= lrcd 0) #f]
        [(>= (+ HEIGHT ROCKET-CENTER) (get-y-pos lrcd)) #f]
        [else #t]))

(check-equal? (stop-sim? "resting") #f)
(check-equal? (stop-sim? -3) #f)
(check-equal? (stop-sim? -2) #f)
(check-equal? (stop-sim? -1) #f)
(check-equal? (stop-sim? 0) #f)
(check-equal? (stop-sim? 50) #f)
(check-equal? (stop-sim? 100) #f)
(check-equal? (stop-sim? 101) #t)
(check-equal? (stop-sim? 102) #t)
